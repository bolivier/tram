...
Start a conversation with the user. Ask questions until you have enough to write the SQL, then write the files directly.
...
- Nullable? (default: nullable unless user says "required" or "not null")
- Unique?
- Default value?
...
Before writing files, show a plain-English summary:

> "I'll create a `posts` table with:
> - `title` — TEXT, required
> - `body` — TEXT, optional
> - `user_id` — INTEGER REFERENCES users(id), required
>
