...
(deftest with-stub
  ...
  (testing "calls are tracked"
    (sut/with-stub [calls fake-fn]
      ...
      (is (match? false @called-for-real?) "fake-fn should not be called")
      (is (match? [{:args nil} {:args [123]} {:args [[123] :foobar :test]}]
                  @calls)
          "calls were incorrect.")))
  ...
  (testing "With dynamic return behavior"
    (sut/with-stub [_ {:fn   fake-fn
      ...
      (is (= 3 (fake-fn 2)))
      (is (= 5 (fake-fn 4)))
      (is (thrown-match? Exception nil (fake-fn nil)))
      (is (match? false @called-for-real?) "fake-fn should not be called")))
  (testing "multiple stubs in one"
    (sut/with-stub [calls-1 fake-fn
      ...
      (is (match? [{:args nil} {:args [123]} {:args [[123] :foobar :test]}]
                  @calls-1)
          "fake-fn calls were incorrect.")
      (is (match? [{:args [2]} {:args ["test" :keyword]}] @calls-2)
          "another-fake-fn calls were incorrect."))))
