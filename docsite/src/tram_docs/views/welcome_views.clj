(ns tram-docs.views.welcome-views
  (:require [tram.vars :refer [*current-user*]]))

(def ^:private next-steps
  [{:title "Add a route"
    :body
    "Routes are defined with defroutes in src/tram_docs/handlers/, then mounted in src/tram_docs/routes.clj."}
   {:title "Add a view"
    :body
    "A :get of :view/welcome renders welcome in src/tram_docs/views/welcome_views.clj. Views take locals and return hiccup."}
   {:title "Add a model"
    :body
    "Run tram db:migrate after writing a migration in resources/migrations/, then add a model under src/tram_docs/models/."}])
...
(defn welcome [_locals]
  [:div {:class "max-w-2xl mx-auto mt-16 mb-8 space-y-8"}
   ...
   [:p {:class "text-sm text-gray-500"}
    "This page lives in src/tram_docs/views/welcome_views.clj and is routed from src/tram_docs/handlers/welcome_handlers.clj."]])
