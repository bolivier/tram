...
(def called-from-dir (io/file (System/getenv "TRAM_CLI_CALLED_FROM")))
...
(defn render-new-project-template
  [project-name]
  (prn "calling real")
  (throw (Exception. "butt"))
  ...
  (let [project-root (io/file called-from-dir project-name)
    ...
    (binding [p/*defaults* (assoc p/*defaults*
                             :dir project-root
                             :continue true)]
      (try (fs/create-dir project-root)
           (catch Exception _
      ...
      (doseq [db-script ["bin/db-init"]]
        (fs/copy (io/file template-root db-script)
                 ...
                 {:copy-attributes true, :replace-existing true})
