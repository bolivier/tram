...
(def expand-header-routes-interceptor
  {:name  ::expand-header-routes-interceptor
   :leave (fn [ctx]
            (let [req    (:request ctx)
                  router (::r/router req)]
              (when-not router
                (throw
                  (ex-info
                    "Cannot expand header routes without router in request."
                    {:source ::expand-header-routes-interceptor})))
  ...
  ([options]
   (-> muuntaja/default-options
       (assoc-in [:formats "text/html"]
                 (tram.html/make-html-formatter (:routes options)))
  ...
  ([default-muuntaja]
   {:name    ::format
    ...
    :compile (fn [{:keys [muuntaja]} _]
               (when-let [prototype (or muuntaja default-muuntaja)]
