(ns tram.db.sqlite
  "SQLite integration for Toucan2 (workarounds for generated keys, booleans, and
  UPDATE returning PKs).

  Canonical Toucan2 ships no SQLite support; without these overrides
  `insert-returning-pk!` / `insert-returning-instance!` return nil on SQLite
  because the driver's `getGeneratedKeys()` is unreliable. Requiring this
  namespace registers the workarounds. It only activates for SQLite
  connections, so it is a no-op for PostgreSQL.

  Intended to be extracted into a standalone Clojars dependency eventually; see
  https://github.com/bolivier/tram issue for tracking."
...
;;;
;;; SQLite doesn't support `UPDATE "table" AS "alias"` or `DELETE FROM "table"
;;; AS "alias"`.
;;; We rewrite the compiled SQL to remove aliases and alias-qualified column
;;; references.
...
(defn- strip-table-alias
  ...
  [^String sql]
  (if-let [[_ _stmt-type table table-alias]
           (re-find
             #"(?i)(UPDATE|DELETE FROM)\s+\"([^\"]+)\"\s+(?:AS\s+)?\"([^\"]+)\""
             sql)]
    (let [quoted-alias (java.util.regex.Pattern/quote table-alias)]
      (-> sql
          (str/replace (str "\"" table-alias "\".") "")
          (str/replace (re-pattern (str "(?i)(\""
...
(defn- maybe-rewrite-sql
  ...
  [compiled-query]
  (if (and (sequential? compiled-query)
           (string? (first compiled-query)))
    (let [sql (first compiled-query)]
      (if (or (str/starts-with? (str/upper-case sql)
                                "UPDATE")
              (str/starts-with? (str/upper-case sql)
                                "DELETE"))
        (into [(strip-table-alias sql)]
              (rest compiled-query))
        compiled-query))
    compiled-query))
...
(m/defmethod pipeline/transduce-execute-with-connection :around
  [#_conn ::connection #_query-type :default #_model :default]
...
;;;; Boolean handling
;;;
;;; SQLite stores booleans as integers (0/1). The JDBC driver reports the type
;;; as Types/BOOLEAN
;;; but returns Integer values. Convert them to proper booleans.
(m/defmethod jdbc.read/read-column-thunk
  [#_conn ::connection #_model :default #_type Types/BOOLEAN]
...
(defn- append-returning-pks
  ...
  (let [pks       (model/primary-keys model)
        returning (str " RETURNING "
                       (str/join ", " (map #(str "\"" (name %) "\"") pks)))
        sql       (first compiled-query)]
    (into [(str sql returning)] (rest compiled-query))))

(m/defmethod pipeline/transduce-execute-with-connection
  [#_conn
   ::connection
   #_query-type
  ...
  (let [rows (:rows pipeline/*parsed-args*)
        ...
        return-pks-directly?
        (and (seq rows)
             (every? (fn [row] (every? (fn [k] (contains? row k)) pks)) rows))]
    (if return-pks-directly?
      (do (pipeline/transduce-execute-with-connection
            (pipeline/default-rf :toucan.query-type/insert.update-count)
            conn
            :toucan.query-type/insert.update-count
            model
            compiled-query)
          (transduce (map (model/select-pks-fn model))
                     rf
                     rows))
      (let [returning-query (append-returning-pks compiled-query
                                                  model)
...
(m/defmethod pipeline/transduce-execute-with-connection
  ...
  (let
    [conditions-map pipeline/*resolved-query*
     _
     (log/debugf
       "SQLite update-returning-pks workaround: doing SELECT with conditions %s"
       conditions-map)

     parsed-args    (update pipeline/*parsed-args*
                            :kv-args
                            merge
                            conditions-map)
     select-rf      (pipeline/conj-with-init! [])
     xform          (map (model/select-pks-fn model))
     pks            (pipeline/transduce-query
                      (xform select-rf)
    ...
    (let [update-rf (pipeline/default-rf
                      :toucan.query-type/update.update-count)]
      (log/debugf
        "SQLite update-returning-pks workaround: performing original UPDATE")
      (pipeline/transduce-execute-with-connection
        update-rf
        conn
        :toucan.query-type/update.update-count
        model
        sql-args))
    (log/debugf
      "SQLite update-returning-pks workaround: transducing PKs with original reducing function")
    (transduce identity original-rf pks)))
